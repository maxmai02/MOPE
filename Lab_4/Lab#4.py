...
def regression_equation(x, y, n):
    ...
    a = [
         (sum([y_average[i] * x[i][1] for i in range(len(x))]) / n),
         (sum([y_average[i] * x[i][2] for i in range(len(x))]) / n),
         (sum([y_average[i] * x[i][3] for i in range(len(x))]) / n)
        ]
    ...
    X = [[1, mx[0], mx[1], mx[2]], [mx[0], a11, a12, a13], [mx[1], a12, a22, a23], [mx[2], a13, a23, a33]]
    Y = [my, a[0], a[1], a[2]]
